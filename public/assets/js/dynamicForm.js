    ...
    $.fn.dynamicForm = function (action, key, value, options) {
        ...
        if (action === "set") {
            ...
            if (register[key].counter == 1) {
                $(register[key].template).clone(settings.withDataAndEvents, settings.deepWithDataAndEvents).find(
                    settings.domElements).each(
                    function () {
                        for (a in settings.domElementAttr) {
                            if ($(this).attr(settings.domElementAttr[a])) {
                                $(this).attr(settings.domElementAttr[a],
                                    $(this).attr(settings.domElementAttr[a]).replace(/\d+/, register[key].uid));
                            }
                        }
                        a = 0;
                        for (a in settings.placeholderAttr) {
                            if ($(this).attr(settings.placeholderAttr[a])) {
                                $(this).attr(settings.placeholderAttr[a],
                                    $(this).attr(settings.placeholderAttr[a]).replace(settings.placeholder,
                                        register[key].id));
                            }
                        }
                        a = 0;
